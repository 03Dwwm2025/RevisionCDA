...
export const questionsBddSql: Question[] = [
  // --- DDL / DML / DCL ---
  {
    ...
    paires: [
      { gauche: 'DDL (définition)', droite: 'CREATE, ALTER, DROP' },
      { gauche: 'DML (manipulation)', droite: 'SELECT, INSERT, UPDATE, DELETE' },
      { gauche: 'DCL (droits)', droite: 'GRANT, REVOKE' },
      { gauche: 'TCL (transactions)', droite: 'BEGIN, COMMIT, ROLLBACK' },
    ],
    explication:
      'Le DDL structure la base, le DML manipule les données, le DCL gère les droits, le TCL contrôle les transactions. Au quotidien on écrit surtout du DML ; le DDL passe le plus souvent par un outil de migration versionné, qui rejoue les changements de schéma dans l’ordre sur chaque environnement.',
  },
  {
    id: 'bdd-002',
    theme: 'bdd-sql',
...
ORDER BY d.dateDebut;`,
  ...
  },
  {
    ...
    difficulte: 1,
    enonce:
      'Une requête paramétrée protège de l’injection SQL, contrairement à une requête construite par concaténation de la saisie.',
    bonneReponse: true,
    explication:
      'Un paramètre est transmis à part du texte de la requête et traité comme une valeur, jamais comme du code : la saisie `\' OR \'1\'=\'1` devient une chaîne recherchée telle quelle. Concaténer laisse l’utilisateur écrire une partie de la requête. Le nom du marqueur change selon le moteur (`:nom`, `@nom` ou `?`) ; un ORM paramètre automatiquement.',
  },
  {
    id: 'bdd-006',
    theme: 'bdd-sql',
  ...
  },
  {
    ...
    difficulte: 2,
    enonce:
      'Une couche d’accès aux données écrite avec un ORM enchaîne un filtre, un tri, puis demande la liste des résultats. Que se passe-t-il ?',
    options: [
      'L’ORM traduit le tout en une requête SQL paramétrée : le filtre et le tri sont exécutés par la base',
      'Toutes les lignes sont chargées en mémoire, puis filtrées et triées par le code applicatif',
      'Une requête est envoyée par condition, puis les résultats sont fusionnés',
      'Une erreur est levée si aucune ligne ne correspond',
    ],
    bonneReponse: 0,
    explication:
      'C’est tout l’intérêt d’un ORM : il compose la requête et ne l’exécute qu’au moment où on demande vraiment les données. Le travail reste côté base, qui sait utiliser ses index. Le piège classique est de ramener la collection complète en mémoire avant de filtrer : ça fonctionne sur dix lignes, ça s’écroule sur cent mille.',
  },
  {
    id: 'bdd-011',
    theme: 'bdd-sql',
    type: 'completer_code',
    difficulte: 3,
    enonce: 'Complétez ce script de création de table avec les contraintes correctes.',
    codeAvecTrous: `CREATE TABLE Demande (
  idDemande  INT ___1___,
  dateDebut  DATE NOT NULL,
  dateFin    DATE NOT NULL,
  statut     VARCHAR(20) NOT NULL ___2___ 'EN_ATTENTE',
  ...
  ___3___ (idSalarie) REFERENCES Salarie(idSalarie)
    ...
    explication:
      'La clé primaire identifie chaque ligne de façon unique. La valeur par défaut évite d’avoir à préciser le statut à chaque insertion. La clé étrangère garantit qu’une demande pointe vers un salarié qui existe. L’auto-incrément s’écrit différemment selon le moteur (`AUTO_INCREMENT`, `SERIAL`, `IDENTITY`), la contrainte elle est standard.',
  },
  {
    id: 'bdd-012',
    theme: 'bdd-sql',
...
___3___ * FROM VueDemandesEnAttente ORDER BY dateDebut;`,
  ...
  },
  {
    ...
    options: [
      'La procédure accepte des paramètres et contient de la logique ; la vue est une requête de lecture mémorisée',
      'La vue accepte des paramètres, pas la procédure stockée',
      'La procédure stockée est plus lente car elle n’est pas mise en cache',
      'Il n’y a aucune différence, ce sont deux noms pour la même chose',
    ],
    bonneReponse: 0,
    explication:
      'Une vue s’interroge comme une table et ne fait que lire. Une procédure stockée est un bloc exécutable : elle prend des paramètres, enchaîne des conditions, écrit en base, et s’appelle explicitement. La vue simplifie la lecture, la procédure centralise un traitement.',
  },
  {
    id: 'bdd-016',
    theme: 'bdd-sql',
